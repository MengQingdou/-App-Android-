...
public class MoreActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.more_tv_share:
                shareSoftwareMag("说天气APP是一款超萌超可爱的天气预报软件，画面简约，播报天气情况非常精准，快来下载吧！");
                break;
    ...
    private void shareSoftwareMag(String s) {
        /*分享软件的函数*/
    ...
    private void clearCache() {
        ...
        builder.setTitle("提示信息").setMessage("确定要删除所有缓存嘛？");
        builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                DBManager.deleteAllInfo();
                Toast.makeText(MoreActivity.this,"已清楚全部缓存",Toast.LENGTH_SHORT).show();
...
//                获取目前的默认壁纸
                ...
                switch (checkedId){
                    case R.id.more_rb_green:
                        if(bg == 0){
                            Toast.makeText(MoreActivity.this, "您选择的为当前背景，无须改变！", Toast.LENGTH_SHORT).show();
                            return;
                        }
                        editor.putInt("bg",0);
                        editor.commit();
